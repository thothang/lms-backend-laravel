...
const AdminUploadEbook = ({ isEmbedded = false, onSuccess = null }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    // Admin-specific: auto approve + doanh thu thuộc thư viện
    data.append('auto_approve', 1);
    data.append('library_revenue', 1);
    data.append('author_name', formData.author_name);
    // Admin đăng: uploaded_by_admin = true
    data.append('uploaded_by_admin', 1);

    try {
      await adminService.uploadEbook(data);
    ...
    <div className="space-y-8">
      ...
      {!isEmbedded && (
        <div className="flex items-center gap-4">
          <Link to="/admin/dashboard" className="p-2 hover:bg-white rounded-xl transition-colors text-slate-400 hover:text-indigo-600">
            <ArrowLeft size={24} />
          </Link>
          <div>
            <h1 className="text-3xl font-black text-slate-800 tracking-tight">Đăng tải Ebook (Admin)</h1>
            <p className="text-slate-500 font-medium mt-1">Đăng ebook với quyền quản trị — tự động xuất bản, không cần duyệt.</p>
          </div>
        </div>
      )}
          ...
          <div className="bg-white p-8 rounded-3xl border border-slate-100 shadow-sm space-y-6">
            ...
            <div>
              ...
              <Input 
                ...
                required
              />
              <p className="text-[11px] text-slate-400 mt-2 leading-relaxed">Tên tác giả hiển thị trên ebook. Doanh thu bán ebook sẽ thuộc về thư viện.</p>
            </div>
          </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <div className="mt-3 flex items-center gap-2">
                     ...
                     />
                     <label htmlFor="is_free_admin" className="text-xs font-bold text-slate-600 cursor-pointer">Đây là sách miễn phí</label>
                  </div>
                </div>
              </div>
        ...
        <div className="space-y-6">
          ...
          <div className="flex gap-4 p-4 bg-emerald-50 rounded-2xl border border-emerald-100">
             ...
             <p className="text-[11px] text-emerald-700 leading-relaxed font-medium">
               Ebook được đăng từ quyền Admin sẽ <strong>tự động xuất bản</strong> mà không cần kiểm duyệt. Toàn bộ doanh thu bán ebook sẽ <strong>thuộc về thư viện</strong>.
             </p>
          </div>
        </div>
      </form>
...
export default AdminUploadEbook;
